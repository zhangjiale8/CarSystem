...
function(a, b) {
    ...
    var c, d, e = {
        getPath: function() {
            ...
            if (!b.getAttribute("merge")) return location;
        } (),
        enter: function(a) {
    ...
    f = {
        ...
        config: function(a, b) {
            ...
            f.use("/resource/plugins/layer/skin/layer.css", a.extend && a.extend.length > 0 ?
            function g() {
                var c = a.extend;
    ...
    g.pt.creat = function() {
        ...
        b.fix && d.on("resize",//如果是三维窗口则窗口小大改变时不自动居中
        function() {
        	if(c("#" + h[0] + g).attr("data-id") == "unityWindow"){
        		return;
        	}
            a.offset(),
    ...
    g.pt.auto = function(a) {
        function b(a) {		//设置窗口主体内容的高度
            a = g.find(a),
            a.height(i[1] - j - k - 2 * ( parseFloat(a.css("padding")) || 0))
        }
        var e = this,
        f = e.config,
        g = c("#" + h[0] + a);
        ...
        j = g.find(h[1]).outerHeight() || 0,			//标题的高度
        k = g.find("." + h[6]).outerHeight() || 0;		//按扭的高度
        switch (f.type) {
        case 2:
            b("iframe");
            break;
    ...
    f.style = function(a, b) {
        var d = c("#" + h[0] + a),
        f = d.attr("type"),
        g = d.find(h[1]).outerHeight() || 0;	//标题高度
        var i = d.find("." + h[6]).outerHeight() || 0; //按扭区域的高度 
        var nHeight = d.find(h[1]).css("display") == "none" ? parseFloat(b.height) - i: parseFloat(b.height) - g - i;
		...
		//如果是page
		(f === e.type[1] || f === e.type[2]) && (d.css(b), b.content && d.find("." + h[5]).css(b.content), f === e.type[1] && d.find("." + h[5]).css({
            height: nHeight
        }));
    },
    f.min = function(a, b) {
    ...
    f.restore = function(a) {
      ...
      //如果当前窗口是三维窗口
        var unity3D = b.find("#unity");
        if(unity3D && unity3D.length != 0){
        	unity3D.find("#unity_view").css("width", "75%");
        	unity3D.find("#unity_info").css("width", "25%");
        }
    },
    f.full = function(a) {
        var b, g = c("#" + h[0] + a);
        ...
        //如果当前窗口是三维窗口
        var unity3D = g.find("#unity");
        if(unity3D && unity3D.length != 0){
        	unity3D.find("#unity_info").css("display", "inline-block");
        	unity3D.find("#unity_view").css("width", "80%");
        	unity3D.find("#unity_info").css("width", "20%");
        }
    },
    f.title = function(a, b) {
        var d = c("#" + h[0] + (b || f.index)).find(h[1]);
        a && (d.css("display", "block").html(a), c("#" + h[0] + (b || f.index)).find(".layui-layer-setwin").css("display", "block"));
        !a && (d.css("display", "none"), c("#" + h[0] + (b || f.index)).find(".layui-layer-setwin").css("display", "none"));
    },
    f.close = function(a) {
        var b = c("#" + h[0] + a),
        d = b.attr("type");
        ...
        //如果是三维窗口则不关关闭,只是移出屏幕
        if(b.find("[data-id='unityWindow']") && b.find("[data-id='unityWindow']").length != 0){
        	f.hide(a);
        	return;
        }
    ...
    f.show = function(a){
    	var b = c("#" + h[0] + a);
    	var left = (c(window).width() - b.width()) / 2,
    	top = (c(window).height() - b.height()) / 2;
		b.css({
			"top": top + "px",
			"left": left + "px"
		});
